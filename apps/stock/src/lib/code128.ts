...
export function internalBarcodeForItemId(itemId: string) {
  return `KAN-${itemId.replace(/[^a-zA-Z0-9]/g, "").toUpperCase()}`;
}
...
export function encodeCode128B(value: string) {
  if (!value || [...value].some((character) => {
    ...
    throw new Error("Le CODE 128 accepte uniquement les caractères ASCII imprimables.");
  }
  ...
  return {
    codes,
    patterns: codes.map((code) => CODE_128_PATTERNS[code]),
  };
}
